...
export function insertRawItem(db, { sourceId, title, url, author, content, publishedAt, dedupKey, metadata }) {
  const key = dedupKey || (url ? `${sourceId}:${url}` : `${sourceId}:${_contentHash(content)}`);
  ...
  try {
    ...
    ).run(sourceId, title || '', url || '', author || '', content || '', publishedAt || null, key, meta);
    return { id: result.lastInsertRowid, inserted: result.changes > 0 };
  } catch (e) {
    if (e.message.includes('UNIQUE constraint')) return { id: null, inserted: false };
    throw e;
  }
}
...
export function touchSourceFetch(db, sourceId) {
  return db.prepare("UPDATE sources SET last_fetched_at = datetime('now'), fetch_count = fetch_count + 1 WHERE id = ?").run(sourceId);
}
